function [nLogL, mo, logL] = modelMF_S0fixed_sgm0fixed(param_val,param_names,sID,settings,data,gameIDs)
...
%         disp([int2str(c) '_' int2str(g)])
        ...
        if g_idx < g && mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g)==mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g_idx)
            ...
            % compute model game
            [logL(c,g),mo] = model_game_MF_lapse(mo,tmp_dat,c,g,settings.opts.TLT);
