function [nMAP, nLogL, mo, logL] = modelMF_S0fixed_Q0fixed_sgm0fixed_MAP_hybrid(param_val,param_names,sID,settings,data,gameIDs)
...
%% initialise model
mo = initialise_model_MF_S0fixed_Q0fixed_sgm0fixed(settings);
...
%         disp([int2str(c) '_' int2str(g)])
        ...
        if g_idx < g && mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g)==mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g_idx)
            ...
            % compute model game
            [logL(c,g),mo] = model_game_MF_no_lapse(mo,tmp_dat,c,g,settings.opts.TLT); % log(0<...<1) = neg 
...
load('participant_data\priors\hybrid_3params\empirical_prior.mat'); %from generate_empirical_priors.m
param_names = {'gamma', 'gamma','tau', 'tau', 'w_hyb', 'w_hyb'};
