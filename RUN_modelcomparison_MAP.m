...
% n=n+1;
% mod(n).desc  = 'Thompson3paramsxiBeta';
% mod(n).fname = strcat('thompson_MAP\3params\results_1Hor_xiBeta\fmincon\res_thompson_');
% 
% n=n+1;
% mod(n).desc  = 'Thompson3paramsxiNorm';
% mod(n).fname = strcat('thompson_MAP\3params\results_1Hor_xiNorm\fmincon\res_thompson_');

n=n+1;
mod(n).desc  = 'Thompson2params';
mod(n).fname = strcat('thompson_MAP\2params\results_1Hor\fmincon\res_thompson_');

% n=n+1;
% mod(n).desc  = 'Thompson3params1HorxiUnif';
% mod(n).fname = strcat('thompson_MAP\3params\results_1Hor_xiUnif\fmincon\res_thompson_');

n=n+1;
mod(n).desc  = 'Thompson3params2HorxiUnif';
mod(n).fname = strcat('thompson_MAP\3params\results_2Hor_xiUnif\fmincon\res_thompson_');

% n=n+1;
% mod(n).desc  = 'UCB3paramsxiBeta';
% mod(n).fname = strcat('UCB_MAP\3params\results_1Hor_xiBeta\fmincon\res_UCB_');
% 
% n=n+1;
% mod(n).desc  = 'UCB3paramsxiNorm';
% mod(n).fname = strcat('UCB_MAP\3params\results_1Hor_xiNorm\fmincon\res_UCB_');
% 
% n=n+1;
% mod(n).desc  = 'UCB2params1Hor';
% mod(n).fname = strcat('UCB_MAP\2params\results_1Hor\fmincon\res_UCB_');
% 
% n=n+1;
% mod(n).desc  = 'UCB3params1HorxiUnif';
% mod(n).fname = strcat('UCB_MAP\3params\results_1Hor_xiUnif\fmincon\res_UCB_');

n=n+1;
mod(n).desc  = 'UCB2params2Hor';
mod(n).fname = strcat('UCB_MAP\2params\results_2Hor\fmincon\res_UCB_');

n=n+1;
mod(n).desc  = 'UCB3params2HorxiUnif';
mod(n).fname = strcat('UCB_MAP\3params\results_2Hor_xiUnif\fmincon\res_UCB_');

n=n+1;
mod(n).desc  = 'Hybrid3param';
mod(n).fname = strcat('hybrid_MAP\3params\results_1Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid3param2Hor';
mod(n).fname = strcat('hybrid_MAP\3params\results_2Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid3paramxi';
mod(n).fname = strcat('hybrid_MAP\3params_xi\results_1Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid3paramxi2Hor';
mod(n).fname = strcat('hybrid_MAP\3params_xi\results_2Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid4param';
mod(n).fname = strcat('hybrid_MAP\4params\results_1Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid4param2Hor';
mod(n).fname = strcat('hybrid_MAP\4params\results_2Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid4paramxi';
mod(n).fname = strcat('hybrid_MAP\5params\results_1Hor\fmincon\res_hybrid_');

n=n+1;
mod(n).desc  = 'Hybrid4paramxi2Hor';
mod(n).fname = strcat('hybrid_MAP\5params\results_2Hor\fmincon\res_hybrid_');
...
%% calculate modelfits
    for n = 1:length(mod)
        ...
        MAP(n,:) = [tmpMat(:).MAP].*-1; % neg LL * -1

        % Det Hessian
        for i =1:npart
            if det(tmpMat(i).He_MAP) < 0         
                logdetHess(n,i) = nan;
                disp(strcat('hess neg for part', int2str(i), 32, 'in', 32, mod(n).desc))
            elseif det(tmpMat(i).He_MAP) == 0   
                logdetHess(n,i) = nan;
                disp(strcat('hess null for part', int2str(i), 32, 'in', 32, mod(n).desc))
            elseif -log(det(tmpMat(i).He_MAP)) > realmax
                logdetHess(n,i) = -realmax;
            else
                logdetHess(n,i) = log(det(tmpMat(i).He_MAP));
            end
        end
        ...
        lme(n,:) = -(MAP(n,:) + 0.5*(n_paramAll*log(2*pi) - logdetHess(n,:))); % posterior = logL * uniform distrib = logL
        desc{n} = mod(n).desc;
    end
...
h = findobj(gca,'Type','BAR');
set(h(1),'FaceColor',CT(:),'EdgeColor','k');
ylabel('Negative averaged marginal likelihood (model evidence)','FontSize',fs,'FontWeight','bold')
