function HOLLY_crossvalidation_hybrid_3param_xi_noveltybonus_2Hor(ID)
    ...
    for k = 1:10
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            algo = 'crossval_hybrid_noveltybonus';
            n_param = 3;
            
            if holly == 1
                results_dir = strcat('/home/mdubois/data/participant_data/',algo,'/',int2str(n_param), 'params_xi_2Hor/results/');
            else
                results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\',algo,'\', int2str(n_param), 'params_xi_2Hor\results\');
            end
            ...
            settings.params.param_names = {'gamma' '' 'tau' '' 'xi' '' 'eta' 'w_hyb' ''};   % is same param name as prev, write ''
            settings.params.lb          = [param_bounds_gamma(1) param_bounds_gamma(1) param_bounds_tau(1) param_bounds_tau(1) param_bounds_xi(1) param_bounds_xi(1) param_bounds_eta(1) param_bounds_w_hyb(1) param_bounds_w_hyb(1)];    % lower bound
            settings.params.ub          = [param_bounds_gamma(2) param_bounds_gamma(2) param_bounds_tau(2) param_bounds_tau(2) param_bounds_xi(2) param_bounds_xi(2) param_bounds_eta(2) param_bounds_w_hyb(2) param_bounds_w_hyb(2)];    % upper bound
            ...
            modelfun = @(x) modelMF_S0fixed_Q0fixed_sgm0fixed_lapse_eta_crossval(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, 180);   
