function cv_mod20_max(ID)
    ...
    for k = 1:Kf
        ...
        if holly == 1
            results_dir = strcat('/home/mdubois/data/dev_participant_data/',algo,'/',int2str(n_param), 'params_eta_xi/results/');
        else
            results_dir = strcat('D:\MaggiesFarm\modeling_05_07_developmental\participant_data\k',int2str(Kf),'\',algo,'\', int2str(n_param), 'params_eta_xi\results\');
        end
        ...
        modelfun = @(x) modelMF_S0fixed_sgm0fixed_crossval_eta_max(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);  
        ...
        [test_nLogL, ~, ~] = modelMF_S0fixed_sgm0fixed_crossval_eta_max(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, trials_tested_on);
