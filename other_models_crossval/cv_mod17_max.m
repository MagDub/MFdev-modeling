function cv_mod17_max(ID)
    ...
    for k = 1:Kf
        ...
        settings.params.param_names = {'Q0'};   % is same param name as prev, write ''
        settings.params.lb          = [param_bounds_Q0(1)];    % lower bound
        settings.params.ub          = [param_bounds_Q0(2)];    % upper bound
        ...
        modelfun = @(x) modelMF_S0fixed_sgm0fixed_crossval_nolapse_max(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);  
        ...
        [test_nLogL, ~, ~] = modelMF_S0fixed_sgm0fixed_crossval_nolapse_max(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, trials_tested_on);
