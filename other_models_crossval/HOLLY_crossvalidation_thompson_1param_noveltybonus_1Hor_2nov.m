function HOLLY_crossvalidation_thompson_1param_noveltybonus_1Hor_2nov(ID)
    ...
    for k = 1:10
        ...
        algo = 'crossval_thompson_noveltybonus';
        n_param = 1;

        if holly == 1
            results_dir = strcat('/home/mdubois/data/participant_data/',algo,'/',int2str(n_param), 'params_1Hor_2nov/results/');
        else
            results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\',algo,'\', int2str(n_param), 'params_1Hor_nov\results\');
        end
        ...
        settings.params.param_names = {'sgm0', 'eta', ''};   % is same param name as prev, write ''
        settings.params.lb          = [param_bounds_sgm0(1)  param_bounds_eta(1) param_bounds_eta(1)];    % lower bound
        settings.params.ub          = [param_bounds_sgm0(2)  param_bounds_eta(2) param_bounds_eta(2)];    % upper bound
        ...
        [test_nLogL, ~, ~] = modelMF_S0fixed_Q0fixed_eta_crossval(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, 20);
