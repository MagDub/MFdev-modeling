function mo = initialise_model_MF_S0fixed_Q0fixed_lapse_beta(settings)
...
mo.params.sgm0          = nan*ones(settings.task.N_games,1); % prior mean std (set up, so that learning can be implemented
...
mo.params.w_hyb         = [];
mo.params.beta1          = nan*ones(settings.task.N_games,1);
mo.params.beta2         = nan*ones(settings.task.N_games,1);
mo.params.beta3         = nan*ones(settings.task.N_games,1);
