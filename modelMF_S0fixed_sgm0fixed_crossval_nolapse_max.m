function [nLogL, mo, logL] = modelMF_S0fixed_sgm0fixed_crossval_nolapse_max(param_val,param_names,sID,settings,data,gameIDs, game_ini, game_fin)



%% initialise model
mo = initialise_model_MF_S0fixed_sgm0fixed_nolapse_max(settings);
...
%        disp([int2str(c) '_' int2str(g)])
        ...
        if g_idx < g && mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g)==mo.params.Q0((c-1)*settings.task.Ngames_per_hor+g_idx)
            ...
            % compute model game
            [logL(c,g),mo] = model_game_MF_no_lapse_max2(mo,tmp_dat,c,g,settings.opts.TLT);
        end
    end
end
