function [logL,mo] = model_game_MF_no_lapse(mo,data,idx_hor,idx_g,TLT)

%% loop through trials of game
for t = 1:size(data.alltrees,1)+1 %Changed
    ...
    if t == 1 % plug in priors
        ...
        mo.mat.sgm{idx_hor,idx_g}(:,t) = mo.params.sgm0(data.gameNo);
    end
    
    if t <= size(data.alltrees,1) %Changed
