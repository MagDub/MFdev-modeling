...
for age_i = 8:18
    ...
    conv_age(:,i) = tmp;
    lm_SH{i} = fitlm([conv_age(:,i)], const_SH);
    lm_LH{i} = fitlm([conv_age(:,i)], const_LH);
    lm_diff{i} = fitlm([conv_age(:,i)], const_diff);
...
for age_i = 1:size(control_age,2)
    lm_control_SH{i} = fitlm([control_age(:,i)], const_SH);
    lm_control_LH{i} = fitlm([control_age(:,i)], const_LH);
    lm_control_diff{i} = fitlm([control_age(:,i)], const_diff);
    i=i+1;
end
...
subplot(2,3,3)
plot(age, const_diff,'o'); hold on;
xlabel('age');
ylabel('consistency - Diff')
...
save('linear_models\consist\emergent_age_SH.mat', 'lm_SH')
save('linear_models\consist\emergent_age_LH.mat', 'lm_LH')
save('linear_models\consist\emergent_age_diff.mat', 'lm_diff')
...
consist_demog_desc = {'Const Age', 'Age', 'Age Squared', 'IQ'};

save('linear_models\consist\demog_SH.mat', 'lm_SH')
save('linear_models\consist\demog_LH.mat', 'lm_LH')
save('linear_models\consist\demog_diff.mat', 'lm_diff')
save('linear_models\demog_desc.mat', 'consist_demog_desc')

% % Save
% out_dir = 'D:\MaggiesFarm\modeling_05_07_developmental\linear_regression\convergent_age\figs\';
% addpath('D:\writing\export_fig')
% export_fig([out_dir 'Fig_consist.tif'],'-nocrop','-r200')
