...
for age_i = 8:18
    ...
    conv_age(:,i) = tmp;
    lm_SH{i} = fitlm([conv_age(:,i)], xi_SH);
    lm_LH{i} = fitlm([conv_age(:,i)], xi_LH);
    lm_diff{i} = fitlm([conv_age(:,i)], xi_diff);
    i=i+1;
...
for age_i = 1:size(control_age,2)
    lm_control_SH{i} = fitlm([control_age(:,i)], xi_SH);
    lm_control_LH{i} = fitlm([control_age(:,i)], xi_LH);
    lm_control_diff{i} = fitlm([control_age(:,i)], xi_diff);
    i=i+1;
end
...
subplot(2,3,1)
plot(age, xi_SH,'o'); hold on;
plot(age, lm_control_SH{3}.Coefficients.Estimate(1)+lm_control_SH{3}.Coefficients.Estimate(2).*age.^2);
xlabel('age');
ylabel('xi - Short horizon')

subplot(2,3,2)
plot(age, xi_LH,'o'); hold on;
plot(age, lm_control_LH{3}.Coefficients.Estimate(1)+lm_control_LH{3}.Coefficients.Estimate(2).*age.^2);
xlabel('age');
ylabel('xi - Long horizon')

subplot(2,3,3)
plot(age, xi_diff,'o'); hold on;
plot(age, lm_control_diff{3}.Coefficients.Estimate(1)+lm_control_diff{3}.Coefficients.Estimate(2).*age.^2);
xlabel('age');
ylabel('xi - Diff')
...
%%
mkdir('linear_models\xi\')

save('linear_models\xi\emergent_age_SH.mat', 'lm_SH')
save('linear_models\xi\emergent_age_LH.mat', 'lm_LH')
save('linear_models\xi\emergent_age_diff.mat', 'lm_diff')
...
xi_demog_desc = {'Const Age', 'Age', 'Age Squared', 'IQ'};

save('linear_models\xi\demog_SH.mat', 'lm_SH')
save('linear_models\xi\demog_LH.mat', 'lm_LH')
save('linear_models\xi\demog_diff.mat', 'lm_diff')
save('linear_models\demog_desc.mat', 'xi_demog_desc')

% % Save
% out_dir = 'D:\MaggiesFarm\modeling_05_07_developmental\linear_regression\convergent_age\figs\';
% addpath('D:\writing\export_fig')
% export_fig([out_dir 'Fig_xi.tif'],'-nocrop','-r200')
