...
firstApple_SH = data.average_first_apple_SH(2:end);
firstApple_SH(todel) = [];

firstApple_LH = data.average_first_apple_LH(2:end);
firstApple_LH(todel) = [];

firstApple_diff = firstApple_LH - firstApple_SH;
...
for age_i = 8:18
    ...
    conv_age(:,i) = tmp;
    lm_SH{i} = fitlm([conv_age(:,i)], firstApple_SH);
    lm_LH{i} = fitlm([conv_age(:,i)], firstApple_LH);
    lm_diff{i} = fitlm([conv_age(:,i)], firstApple_diff);
    i=i+1;
...
for age_i = 1:size(control_age,2)
    lm_control_SH{i} = fitlm([control_age(:,i)], firstApple_SH);
    lm_control_LH{i} = fitlm([control_age(:,i)], firstApple_LH);
    lm_control_diff{i} = fitlm([control_age(:,i)], firstApple_diff);
    i=i+1;
end
...
subplot(2,3,1)
plot(age, firstApple_SH,'o'); hold on;
% plot(age, lm_SH{i}.Coefficients.Estimate(1)+lm_SH{i}.Coefficients.Estimate(2)*conv_age(:,10));
xlabel('age');
ylabel('first Apple - Short horizon')

subplot(2,3,2)
plot(age, firstApple_LH,'o'); hold on;
% plot(age, lm_LH{i}.Coefficients.Estimate(1)+lm_LH{i}.Coefficients.Estimate(2)*conv_age(:,10));
xlabel('age');
ylabel('first Apple - Long horizon')

subplot(2,3,3)
plot(age, firstApple_diff,'o'); hold on;
% plot(age, lm_diff{i}.Coefficients.Estimate(1)+lm_diff{i}.Coefficients.Estimate(2)*conv_age(:,4));
xlabel('age');
ylabel('first Apple - Diff')
...
%%
mkdir('linear_models\firstApp\')

save('linear_models\firstApp\emergent_age_SH.mat', 'lm_SH')
save('linear_models\firstApp\emergent_age_LH.mat', 'lm_LH')
save('linear_models\firstApp\emergent_age_diff.mat', 'lm_diff')
...
firstApp_demog_desc = {'Const Age', 'Age', 'Age Squared', 'IQ'};

save('linear_models\firstApp\demog_SH.mat', 'lm_SH')
save('linear_models\firstApp\demog_LH.mat', 'lm_LH')
save('linear_models\firstApp\demog_diff.mat', 'lm_diff')
save('linear_models\demog_desc.mat', 'firstApp_demog_desc')
%%

% out_dir = 'D:\MaggiesFarm\modeling_05_07_developmental\linear_regression\convergent_age\figs\';
% addpath('D:\writing\export_fig')
% export_fig([out_dir 'Fig_firstApple.tif'],'-nocrop','-r200')
