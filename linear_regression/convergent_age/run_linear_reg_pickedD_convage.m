...
for age_i = 8:18
    ...
    conv_age(:,i) = tmp;
    lm_SH{i} = fitlm([conv_age(:,i)], pickedD_SH);
    lm_LH{i} = fitlm([conv_age(:,i)], pickedD_LH);
    lm_diff{i} = fitlm([conv_age(:,i)], pickedD_diff);
    i=i+1;
...
for age_i = 1:size(control_age,2)
    lm_control_SH{i} = fitlm([control_age(:,i)], pickedD_SH);
    lm_control_LH{i} = fitlm([control_age(:,i)], pickedD_LH);
    lm_control_diff{i} = fitlm([control_age(:,i)], pickedD_diff);
    i=i+1;
end
...
plot(age, lm_control_SH{3}.Coefficients.Estimate(1)+lm_control_SH{3}.Coefficients.Estimate(2).*age.^2);
xlabel('age');
ylabel('Picked D - Short horizon')

subplot(2,3,2)
plot(age, pickedD_LH,'o'); hold on;
plot(age, lm_control_LH{3}.Coefficients.Estimate(1)+lm_control_LH{3}.Coefficients.Estimate(2).*age.^2);xlabel('age');
ylabel('Picked D - Long horizon')

subplot(2,3,3)
plot(age, pickedD_diff,'o'); hold on;
plot(age, lm_control_diff{3}.Coefficients.Estimate(1)+lm_control_diff{3}.Coefficients.Estimate(2).*age.^2);xlabel('age');
ylabel('Picked D - Diff')
...
%%
mkdir('linear_models\pickedD\')

save('linear_models\pickedD\emergent_age_SH.mat', 'lm_SH')
save('linear_models\pickedD\emergent_age_LH.mat', 'lm_LH')
save('linear_models\pickedD\emergent_age_diff.mat', 'lm_diff')
...
pickedD_demog_desc = {'Const Age', 'Age', 'Age Squared', 'IQ'};

save('linear_models\pickedD\demog_SH.mat', 'lm_SH')
save('linear_models\pickedD\demog_LH.mat', 'lm_LH')
save('linear_models\pickedD\demog_diff.mat', 'lm_diff')
save('linear_models\demog_desc.mat', 'pickedD_demog_desc')

%%
% out_dir = 'D:\MaggiesFarm\modeling_05_07_developmental\linear_regression\convergent_age\figs\';
% addpath('D:\writing\export_fig')
% export_fig([out_dir 'Fig_pickedD.tif'],'-nocrop','-r200')
