...
highValue_SH = data.highValue_SH_perc(2:end);
highValue_SH(todel) = [];

highValue_LH = data.highValue_LH_perc(2:end);
highValue_LH(todel) = [];

highValue_diff = highValue_LH - highValue_SH;
...
for age_i = 8:18
    ...
    conv_age(:,i) = tmp;
    lm_SH{i} = fitlm([conv_age(:,i)], highValue_SH);
    lm_LH{i} = fitlm([conv_age(:,i)], highValue_LH);
    lm_diff{i} = fitlm([conv_age(:,i)], highValue_diff);
    i=i+1;
...
for age_i = 1:size(control_age,2)
    lm_control_SH{i} = fitlm([control_age(:,i)], highValue_SH);
    lm_control_LH{i} = fitlm([control_age(:,i)], highValue_LH);
    lm_control_diff{i} = fitlm([control_age(:,i)], highValue_diff);
    i=i+1;
end
...
subplot(2,3,1)
plot(age, highValue_SH,'o'); hold on;
% plot(age, lm_SH{i}.Coefficients.Estimate(1)+lm_SH{i}.Coefficients.Estimate(2)*conv_age(:,2));
xlabel('age');
ylabel('highValue- Short horizon')

subplot(2,3,2)
plot(age, highValue_LH,'o'); hold on;
% plot(age, lm_LH{i}.Coefficients.Estimate(1)+lm_LH{i}.Coefficients.Estimate(2)*conv_age(:,4));
xlabel('age');
ylabel('highValue- Long horizon')

subplot(2,3,3)
plot(age, highValue_diff,'o'); hold on;
% plot(age, lm_diff{i}.Coefficients.Estimate(1)+lm_diff{i}.Coefficients.Estimate(2)*conv_age(:,2));
xlabel('age');
ylabel('highValue- Diff')
...
%%
mkdir('linear_models\highValue\')

save('linear_models\highValue\emergent_age_SH.mat', 'lm_SH')
save('linear_models\highValue\emergent_age_LH.mat', 'lm_LH')
save('linear_models\highValue\emergent_age_diff.mat', 'lm_diff')
...
highValue_demog_desc = {'Const Age', 'Age', 'Age Squared', 'IQ'};

save('linear_models\highValue\demog_SH.mat', 'lm_SH')
save('linear_models\highValue\demog_LH.mat', 'lm_LH')
save('linear_models\highValue\demog_diff.mat', 'lm_diff')
save('linear_models\demog_desc.mat', 'highValue_demog_desc')
%%

% out_dir = 'D:\MaggiesFarm\modeling_05_07_developmental\linear_regression\convergent_age\figs\';
% addpath('D:\writing\export_fig')
% export_fig([out_dir 'Fig_highValue.tif'],'-nocrop','-r200')
