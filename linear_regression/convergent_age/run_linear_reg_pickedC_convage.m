...
for age_i = 8:18
    ...
    conv_age(:,i) = tmp;
    lm_SH{i} = fitlm([conv_age(:,i)], pickedC_SH);
    lm_LH{i} = fitlm([conv_age(:,i)], pickedC_LH);
    lm_diff{i} = fitlm([conv_age(:,i)], pickedC_diff);
    i=i+1;
...
for age_i = 1:size(control_age,2)
    lm_control_SH{i} = fitlm([control_age(:,i)], pickedC_SH);
    lm_control_LH{i} = fitlm([control_age(:,i)], pickedC_LH);
    lm_control_diff{i} = fitlm([control_age(:,i)], pickedC_diff);
    i=i+1;
end
...
% plot(age, lm_LH{i}.Coefficients.Estimate(1)+lm_LH{i}.Coefficients.Estimate(2)*conv_age(:,2));
xlabel('age');
ylabel('picked C - Long horizon')

subplot(2,3,3)
plot(age, pickedC_diff,'o'); hold on;
% plot(age, lm_diff{i}.Coefficients.Estimate(1)+lm_diff{i}.Coefficients.Estimate(2)*conv_age(:,2));
xlabel('age');
ylabel('picked C - Diff')
...
%%
mkdir('linear_models\pickedC\')

save('linear_models\pickedC\emergent_age_SH.mat', 'lm_SH')
save('linear_models\pickedC\emergent_age_LH.mat', 'lm_LH')
save('linear_models\pickedC\emergent_age_diff.mat', 'lm_diff')
...
pickedC_demog_desc = {'Const Age', 'Age', 'Age Squared', 'IQ'};

save('linear_models\pickedC\demog_SH.mat', 'lm_SH')
save('linear_models\pickedC\demog_LH.mat', 'lm_LH')
save('linear_models\pickedC\demog_diff.mat', 'lm_diff')
save('linear_models\demog_desc.mat', 'pickedC_demog_desc')
%%

% out_dir = 'D:\MaggiesFarm\modeling_05_07_developmental\linear_regression\convergent_age\figs\';
% addpath('D:\writing\export_fig')
% export_fig([out_dir 'Fig_pickedC.tif'],'-nocrop','-r200')
