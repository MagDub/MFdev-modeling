function HOLLY_cv_mod11_thompson_2param_nov_2Hor_2nov_Q01(ID)
    ...
    for k = 1:Kf
        ...
        if holly == 1
            results_dir = strcat('/home/mdubois/data/dev_participant_data/',algo,'/',int2str(n_param), 'params_2Hor_Q01/results/');
        else
            results_dir = strcat('D:\MaggiesFarm\modeling_05_07_developmental\participant_data\',algo,'\', int2str(n_param), 'params_2Hor_Q01\results\');
        end
        ...
        settings.params.lb          = [param_bounds_sgm0(1)  param_bounds_Q0(1)  param_bounds_eta(1) param_bounds_eta(1)];    % lower bound
        settings.params.ub          = [param_bounds_sgm0(2)  param_bounds_Q0(2)  param_bounds_eta(2) param_bounds_eta(2)];    % upper bound
        ...
        modelfun = @(x) modelMF_S0fixed_crossval_eta(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);  
