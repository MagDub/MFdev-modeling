function HOLLY_cv_mod3_hybrid_4param_nov_both_2Hor_1w_2eta(ID)
    ...
    for k = 1:Kf
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if holly == 1
                results_dir = strcat('/home/mdubois/data/dev_participant_data/',algo,'/',int2str(n_param), 'params_2Hor_1w_2eta/results/');
            else
                results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\',algo,'\', int2str(n_param), 'params_2Hor_1w_2eta\results\');
            end
            ...
            settings.params.param_names = {'gamma' '' 'tau' '' 'sgm0' 'eta' '' 'w_hyb'};   % is same param name as prev, write ''
            settings.params.lb          = [param_bounds_gamma(1) param_bounds_gamma(1) param_bounds_tau(1) param_bounds_tau(1) param_bounds_sgm0(1) param_bounds_eta(1) param_bounds_eta(1) param_bounds_w_hyb(1)];    % lower bound
            settings.params.ub          = [param_bounds_gamma(2) param_bounds_gamma(2) param_bounds_tau(2) param_bounds_tau(2) param_bounds_sgm0(2) param_bounds_eta(2) param_bounds_eta(2) param_bounds_w_hyb(2)];    % upper bound
            ...
            modelfun = @(x) modelMF_S0fixed_Q0fixed_eta_crossval(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);   
