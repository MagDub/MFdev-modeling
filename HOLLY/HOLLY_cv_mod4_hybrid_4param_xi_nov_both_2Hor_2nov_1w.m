function HOLLY_cv_mod4_hybrid_4param_xi_nov_both_2Hor_2nov_1w(ID)
    ...
    for k = 1:Kf
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            algo = 'crossval_hybrid_noveltybonus_both_2nov';
            n_param = 5;
            
            if holly == 1
                results_dir = strcat('/home/mdubois/data/dev_participant_data/',algo,'/',int2str(n_param), 'params_2Hor_1w/results/');
            else
                results_dir = strcat('D:\MaggiesFarm\modeling_28_02\participant_data\',algo,'\', int2str(n_param), 'params_2Hor_1w\results\');
            end
            ...
            settings.params.lb          = [param_bounds_gamma(1) param_bounds_gamma(1) param_bounds_tau(1) param_bounds_tau(1) param_bounds_xi(1) param_bounds_xi(1) param_bounds_sgm0(1)  param_bounds_eta(1) param_bounds_eta(1) param_bounds_w_hyb(1)];    % lower bound
            settings.params.ub          = [param_bounds_gamma(2) param_bounds_gamma(2) param_bounds_tau(2) param_bounds_tau(2) param_bounds_xi(2) param_bounds_xi(2) param_bounds_sgm0(2)  param_bounds_eta(2) param_bounds_eta(2) param_bounds_w_hyb(2)];    % upper bound
            ...
            modelfun = @(x) modelMF_S0fixed_Q0fixed_lapse_eta_crossval(x,settings.params.param_names,ID,settings,data_train,gameIDs_train, 1, trials_trained_on);   
            ...
            [test_nLogL, ~, ~] = modelMF_S0fixed_Q0fixed_sgm0fixed_eta_crossval(mEparams,settings.params.param_names,ID,settings,data_test,gameIDs_test, 1, trials_tested_on);
