function cv_mod5_UCB_2param_2Hor_Q01(ID)
    ...
    for k = 1:Kf
        ...
        algo = 'mod_5';
        results_dir = strcat('D:\MaggiesFarm\modeling_05_07_developmental\participant_data\cv_k',int2str(Kf),'\',algo,'\results\');
            ...
            settings.params.param_names = {'Q0' 'gamma'     ''    'tau' ''};   % is same param name as prev, write ''
            settings.params.lb          = [param_bounds_Q0(1) param_bounds_gamma(1) param_bounds_gamma(1)  param_bounds_tau(1) param_bounds_tau(1)];    % lower bound
            settings.params.ub          = [param_bounds_Q0(2) param_bounds_gamma(2) param_bounds_gamma(2)  param_bounds_tau(2) param_bounds_tau(2)];    % upper bound
